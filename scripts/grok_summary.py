...
def request_grok_summary(transcript, source_lang=None, target_lang=None):
...
def save_output(video_id, content, source_lang=None, target_lang=None, success=True):
    ...
    filename = f"summaries/grok_{video_id}"
    if source_lang:
...
def main():
    parser = argparse.ArgumentParser(description='Generate YouTube video summary using Grok')
    ...
    summary = request_grok_summary(transcript, source_lang, target_lang)
    if summary:
        print(summary)
